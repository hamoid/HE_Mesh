...
public class WB_GeometryOp2D extends WB_GeometryOpGLU {
	 ...
	 * @return
	 */
	public static final WB_IntersectionResult getIntersection2D(final WB_Segment S1, final WB_Segment S2) {
		final double a1 = WB_GeometryOp3D.twiceSignedTriArea2D(S1.getOrigin(), S1.getEndpoint(), S2.getEndpoint());
		final double a2 = WB_GeometryOp3D.twiceSignedTriArea2D(S1.getOrigin(), S1.getEndpoint(), S2.getOrigin());
		if (!WB_Epsilon.isZero(a1) && !WB_Epsilon.isZero(a2) && a1 * a2 < 0) {
			final double a3 = WB_GeometryOp3D.twiceSignedTriArea2D(S2.getOrigin(), S2.getEndpoint(), S1.getOrigin());
	 ...
	 * @param i
	 */
	public static final void getIntersection2DInto(final WB_Segment S1, final WB_Segment S2,
	...
	public static final WB_Segment[] splitSegment2D(final WB_Segment S, final WB_Line L) {
		...
		if (ir2D.dimension == 0) {
			...
			if (WB_GeometryOp3D.classifyPointToLine2D(S.getOrigin(), L) == WB_Classification.FRONT) {
				result[0] = new WB_Segment(S.getOrigin(), (WB_Point) ir2D.object);
				result[1] = new WB_Segment((WB_Point) ir2D.object, S.getEndpoint());
			} else if (WB_GeometryOp3D.classifyPointToLine2D(S.getOrigin(), L) == WB_Classification.BACK) {
	...
	public static final WB_Polygon[] splitPolygon2D(final WB_Polygon poly, final WB_Line L) {
		...
		if (numVerts > 0) {
			...
			WB_Classification aSide = WB_GeometryOp3D.classifyPointToLine2D(a, L);
			WB_Coord b;
	...
	public static final ArrayList<WB_Point> getIntersection2D(final WB_Line L, final WB_Circle C) {
		...
		final double c = WB_CoordOp2D.getSqLength2D(C.getCenter()) + WB_Vector.getSqLength2D(L.getOrigin())
	...
	public static final ArrayList<WB_Point> getIntersection2D(final WB_Ray R, final WB_Circle C) {
		...
		final double c = WB_CoordOp2D.getSqLength2D(C.getCenter()) + WB_Vector.getSqLength2D(R.getOrigin())
	 ...
	 * @return
	 */
	public static final WB_IntersectionResult getClosestPoint2D(final WB_Segment S1, final WB_Segment S2) {
	 ...
	 * @return
	 */
	public static final WB_IntersectionResult getClosestPoint2D(final WB_Line L1, final WB_Line L2) {
	 ...
	 * @return
	 */
	public static final WB_IntersectionResult getClosestPoint2D(final WB_Line L, final WB_Segment S) {
	 ...
	 * @return
	 */
	public static final WB_IntersectionResult getClosestPoint2D(final WB_Segment S, final WB_Line L) {
	...
	public static final WB_Classification classifyPointToCircle2D(final WB_Coord p, final WB_Circle C) {
		final double dist = WB_GeometryOp3D.getDistanceToPoint2D(p, C.getCenter());
	...
	public static final WB_Classification classifyCircleToCircle2D(final WB_Circle C1, final WB_Circle C2) {
		...
		final double dist = WB_GeometryOp3D.getDistanceToPoint2D(C1.getCenter(), C2.getCenter());
	...
	public static final boolean getLineIntersectionInto2D(final WB_Coord a1, final WB_Coord a2, final WB_Coord b1,
		...
		if (Math.abs(det) <= WB_Epsilon.EPSILON) {
			...
			det = 1.0 / det;
			double t2 = det * (WB_CoordOp2D.cross2D(a1, s1) - WB_CoordOp2D.cross2D(b1, s1));
	...
	public static final WB_CircleFit fitCircleToPoints2D(final Collection<? extends WB_Coord> points,
			final int maxIterations, final double convergence) {
		WB_Circle circle = fitCircleToPoints2D(points);

		double invNumPoints = 1.0 / points.size();
		int iteration = 0;
		for (iteration = 0; iteration < maxIterations; iteration++) {
			WB_Coord current = new WB_Point(circle.getCenter());
			double lenAverage = 0;
			for (WB_Coord p : points) {
				WB_Vector diff = WB_Vector.sub(p, circle.getCenter());
				lenAverage += diff.getLength2D();
			}
			...
			circle.setCenter(WB_Point.add(circle.getCenter(), derLenAverage));
			circle.setRadius(lenAverage);
			WB_Vector diff = WB_Vector.sub(circle.getCenter(), current);

			if (diff.getLength2D() <= convergence) {

				break;
			}
		}
		return new WB_CircleFit(circle, iteration);
	...
	public static final WB_CircleFit fitCircleToPoints2D(final WB_Coord[] points, final int maxIterations,
	...
	static final public class WB_CircleFit {
		public WB_Circle circle;
		public int iterations;

		public WB_CircleFit(final WB_Circle C, final int iterations) {
			this.circle = C;
