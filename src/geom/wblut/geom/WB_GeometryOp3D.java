...
public class WB_GeometryOp3D extends WB_GeometryOp2D {
	...
	public static WB_IntersectionResult getIntersection3D(final WB_Ray R, final WB_AABB aabb) {
		...
		i.dimension = 0;
		i.sqDist = WB_CoordOp3D.getSqDistance3D(p, (WB_Point) i.object);
		return i;
	}
	...
	public static boolean checkIntersection3D(final WB_Triangle T, final WB_Sphere S) {
		...
		return p.subToVector3D(S.getCenter()).getSqLength() <= S.getRadius() * S.getRadius();
	}
	 ...
	 * @return
	 */
	public static List<WB_AABBNode> getIntersection3D(final WB_Ray R, final WB_AABBTree tree) {
	 ...
	 * @return
	 */
	public static List<WB_AABBNode> getIntersection3D(final WB_AABB aabb, final WB_AABBTree tree) {
		final List<WB_AABBNode> result = new ArrayList<WB_AABBNode>();
		final LinkedList<WB_AABBNode> queue = new LinkedList<WB_AABBNode>();
		queue.add(tree.getRoot());
		WB_AABBNode current;
	 ...
	 * @return
	 */
	public static List<WB_AABBNode> getIntersection3D(final WB_Line L, final WB_AABBTree tree) {
		final List<WB_AABBNode> result = new ArrayList<WB_AABBNode>();
		final LinkedList<WB_AABBNode> queue = new LinkedList<WB_AABBNode>();
		queue.add(tree.getRoot());
		WB_AABBNode current;
	 ...
	 * @return
	 */
	public static List<WB_AABBNode> getIntersection3D(final WB_Segment S, final WB_AABBTree tree) {
		final List<WB_AABBNode> result = new ArrayList<WB_AABBNode>();
		final LinkedList<WB_AABBNode> queue = new LinkedList<WB_AABBNode>();
		queue.add(tree.getRoot());
		WB_AABBNode current;
	 ...
	 * @return
	 */
	public static List<WB_AABBNode> getIntersection3D(final WB_Plane P, final WB_AABBTree tree) {
		final List<WB_AABBNode> result = new ArrayList<WB_AABBNode>();
		final LinkedList<WB_AABBNode> queue = new LinkedList<WB_AABBNode>();
		queue.add(tree.getRoot());
		WB_AABBNode current;
	 ...
	 * @return
	 */
	public static List<WB_AABBNode> getIntersection3D(final WB_Triangle T, final WB_AABBTree tree) {
		final List<WB_AABBNode> result = new ArrayList<WB_AABBNode>();
		final LinkedList<WB_AABBNode> queue = new LinkedList<WB_AABBNode>();
		queue.add(tree.getRoot());
		WB_AABBNode current;
	...
	public static WB_IntersectionResult getIntersection3D(final WB_Segment S1, final WB_Segment S2) {
		...
		if (!WB_Epsilon.isZero(denom)) {
			...
			if (s2 < e1) {
				...
				if (WB_Epsilon.isZeroSq(getSqDistance3D(S2.getOrigin(), L1))) {
					...
					final WB_IntersectionResult i = new WB_IntersectionResult();
					i.sqDist = WB_CoordOp3D.getSqDistance3D(start, end);
					i.intersection = true;
		...
		i.sqDist = WB_CoordOp3D.getSqDistance3D(p1, p2);
		i.intersection = WB_Epsilon.isZeroSq(i.sqDist);
		if (i.intersection) {
			i.dimension = 0;
			i.object = p1;
	...
	public static WB_Point getClosestPoint3D(final WB_Coord p, final WB_PolyLine PL) {
		...
		for (int s = 0; s < PL.getNumberSegments(); s++) {
			...
			d2 = WB_CoordOp3D.getSqDistance3D(p, test);
			if (d2 < d2min) {
				d2min = d2;
				q = test;
			}
	...
	public static WB_Point getClosestPointOnPeriphery3D(final WB_Coord p, final WB_Triangle T) {
		...
		final double dA2 = WB_CoordOp3D.getSqDistance3D(p, A);
		final WB_Point B = getClosestPointToSegment3D(p, T.p1(), T.p3());
		final double dB2 = WB_CoordOp3D.getSqDistance3D(p, B);
		final WB_Point C = getClosestPointToSegment3D(p, T.p1(), T.p2());
		final double dC2 = WB_CoordOp3D.getSqDistance3D(p, C);
		if (dA2 < dB2 && dA2 < dC2) {
			return A;
	...
	public static WB_IntersectionResult getClosestPoint3D(final WB_Line L1, final WB_Line L2) {
		...
		if (WB_Epsilon.isZero(denom)) {
			...
			final double d2 = WB_CoordOp3D.getSqDistance3D(L1.getOrigin(), p2);
			final WB_IntersectionResult i = new WB_IntersectionResult();
			i.intersection = false;
			i.t1 = 0;
			i.t2 = t2;
		...
		final WB_Point p2 = new WB_Point(L2.getPoint(t2));
		final double d2 = WB_CoordOp3D.getSqDistance3D(p1, p2);
	 ...
	 * @return
	 */
	public static WB_IntersectionResult getIntersection3D(final WB_Coord p1, final WB_Coord p2, final WB_Coord p3,
	...
	public static WB_Point getClosestPoint3D(final WB_Coord p, final WB_Polygon poly) {
		...
		for (int i = 0; i < n; i += 3) {
			...
			final double d2 = WB_CoordOp3D.getSqDistance3D(tmp, q);
			if (d2 < dmin2) {
				closest = tmp;
				dmin2 = d2;
			}
	...
	public static double getDistance3D(final WB_Segment S, final WB_Segment T) {
		return Math.sqrt(WB_GeometryOp3D.getIntersection3D(S, T).sqDist);
	}
	...
	public static double getSqDistance3D(final WB_Segment S, final WB_Segment T) {
		return WB_GeometryOp3D.getIntersection3D(S, T).sqDist;
	}
	...
	public static double getSqDistance3D(final WB_Coord p, final WB_PolyLine PL) {
		...
		double d2;
		for (int s = 0; s < PL.getNumberSegments(); s++) {
			d2 = WB_GeometryOp3D.getSqDistance3D(p, PL.getSegment(s));
			if (d2 < d2min) {
				d2min = d2;
			}
		}
	...
	public static double getSqDistance3D(final WB_Coord p, final WB_Polygon poly) {
		...
		for (int i = 0; i < n; i += 3) {
			...
			tmp = WB_GeometryOp3D.getClosestPointToTriangle3D(p, poly.getPoint(T[i]), poly.getPoint(T[i + 1]),
					poly.getPoint(T[i + 2]));
			final double d2 = WB_CoordOp3D.getDistance3D(tmp, p);
			if (d2 < dmax2) {
				dmax2 = d2;
				if (WB_Epsilon.isZeroSq(dmax2)) {
					break;
	...
	public static WB_Classification classifyPointToPlaneFast3D(final WB_Plane P, final WB_Coord p) {
		final double signp = WB_GeometryOp3D.signedDistanceToPlane3D(p, P);
	...
	public static WB_Classification classifyPointToPlane3D(final WB_Plane P, final WB_Coord p) {
		if (WB_Epsilon.isZero(WB_GeometryOp3D.getDistanceToPlane3D(p, P))) {
			return WB_Classification.ON;
		}
	...
	public static WB_Classification classifyPolygonToPlaneFast3D(final WB_Polygon poly, final WB_Plane P) {
		...
		double d;
		for (int i = 0; i < poly.getNumberOfPoints(); i++) {
			d = WB_GeometryOp3D.signedDistanceToPlane3D(poly.getPoint(i), P);
	...
	public static boolean pointInTriangleBary3D(final WB_Coord p, final WB_Coord A, final WB_Coord B,
		...
		if (WB_Epsilon.isZeroSq(WB_GeometryOp3D.getSqDistanceToLine2D(A, B, C))) {
			return false;
		}
		// Compute vectors
	...
	public static boolean isEqual(final WB_Plane P, final WB_Plane Q) {
		if (!WB_Epsilon.isZero(WB_GeometryOp3D.getDistance3D(P.getOrigin(), Q))) {
			return false;
		}
		if (!WB_Epsilon.isZero(WB_GeometryOp3D.getDistance3D(Q.getOrigin(), P))) {
			return false;
		}
	...
	public static WB_Point getClosestPoint3D(final WB_Coord p, final WB_Circle circle) {
		...
		WB_Point qmc = pmc.sub(WB_Point.mul(circle.getNormal(), pmc.dot(circle.getNormal())));
	...
	public static WB_Sphere fitSphereToPoints(final Collection<? extends WB_Coord> points) {
		...
		double invNumPoints = 1.0 / points.size();
		for (int row = 0; row < 5; ++row) {
	...
	public static WB_SphereFit fitSphereToPoints(final Collection<? extends WB_Coord> points, final int maxIterations,
			final double convergence) {
		WB_Sphere sphere = fitSphereToPoints(points);

		double invNumPoints = 1.0 / points.size();
		int iteration = 0;
		for (iteration = 0; iteration < maxIterations; iteration++) {
			WB_Coord current = new WB_Point(sphere.getCenter());
			double lenAverage = 0;
			for (WB_Coord p : points) {
				WB_Vector diff = WB_Vector.sub(p, sphere.getCenter());
				lenAverage += diff.getLength();
			}
			lenAverage *= invNumPoints;

			WB_Vector derLenAverage = new WB_Vector();
			for (WB_Coord p : points) {
				WB_Vector diff = WB_Vector.sub(p, sphere.getCenter());
				double length = diff.normalizeSelf();
				derLenAverage.addMulSelf(length - lenAverage, diff);
			}

			derLenAverage.mulSelf(invNumPoints);

			sphere.setCenter(WB_Point.add(sphere.getCenter(), derLenAverage));
			sphere.setRadius(lenAverage);
			WB_Vector diff = WB_Vector.sub(sphere.getCenter(), current);

			if (diff.getLength() <= convergence) {

				break;
			}
		}
		return new WB_SphereFit(sphere, iteration);
	...
	public static WB_Sphere fitSphereToPoints(final WB_Coord[] points) {
		...
		double invNumPoints = 1.0 / points.length;
		for (int row = 0; row < 5; ++row) {
			for (int col = 0; col < 5; ++col) {
				A.set(row, col, A.get(row, col) * invNumPoints);
			}
	...
	public static WB_SphereFit fitSphereToPoints(final WB_Coord[] points, final int maxIterations,
			final double convergence) {
		WB_Sphere sphere = fitSphereToPoints(points);
	...
	static public class WB_SphereFit {
		public WB_Sphere sphere;
		public int iterations;

		public WB_SphereFit(final WB_Sphere sphere, final int iterations) {
			this.sphere = sphere;
		 ...
		 * @return
		 */
		public static double[] cross(final double ux, final double uy, final double uz, final double vx,
		 ...
		 * @return
		 */
		public static double[] cross(final double cx, final double cy, final double cz, final double px,
		...
		public static boolean isCollinear(final WB_Coord o, final WB_Coord p, final WB_Coord q) {
			if (WB_Epsilon.isZeroSq(WB_GeometryOp3D.getSqDistanceToPoint3D(p, q))) {
				return true;
			}
			if (WB_Epsilon.isZeroSq(WB_GeometryOp3D.getSqDistanceToPoint3D(o, q))) {
				return true;
			}
			if (WB_Epsilon.isZeroSq(WB_GeometryOp3D.getSqDistanceToPoint3D(o, p))) {
				return true;
			}
	...
	public static WB_Coord getClosestPoint3D(final List<? extends WB_Plane> planes, final double offset) {
		WB_Coord result = null;
		int n = planes.size();
		if (n < 3) {
			return null;
		}
