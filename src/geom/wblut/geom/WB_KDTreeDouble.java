 ...
 *            value type
 */
public class WB_KDTreeDouble<T extends WB_Coord> {
	/** dim. */
	 ...
	 *
	 */
	public WB_KDTreeDouble() {
		this._dim = 3;
		this._maximumBinSize = 32;
		this.root = new WB_KDNodeDouble<T>();
	}
	 ...
	 * @param binsize
	 */
	public WB_KDTreeDouble(final int binsize) {
		this._dim = 3;
	...
	public List<WB_AABB> getLeafBounds() {
		final List<WB_AABB> leafs = new ArrayList<WB_AABB>();
	...
	public List<WB_AABB> getAllBounds() {
		final List<WB_AABB> all = new ArrayList<WB_AABB>();
		root.addBox(all, 0);
		return all;
	}
	...
	public double add(final T coord, final double val) {
		return root.add(new WB_KDEntryDouble<T>(coord, val, -1));
	}
	 ...
	 * @return
	 */
	public WB_KDEntryDouble<T>[] getRange(final WB_AABB aabb) {
	...
	public WB_KDEntryDouble<T>[] getNearestNeighbors(final WB_Coord coord, final int num) {
		final QueryResultDouble<T> heap = new QueryResultDouble<T>(num);
	...
	public WB_KDEntryDouble<T> getNearestNeighbor(final WB_Coord coord) {
		final QueryResultDouble<T> heap = new QueryResultDouble<T>(1);
		root.findNearest(heap, coord);
	 ...
	 * @return
	 */
	public WB_KDEntryDouble<T> getNearestNeighbor(final double x, final double y, final double z) {
		final QueryResultDouble<T> heap = new QueryResultDouble<T>(1);
	...
	private class WB_KDNodeDouble<T extends WB_Coord> {
		...
		private double lookup(final WB_Coord point) {
			for (int i = 0; i < _binSize; i++) {
				if (WB_Epsilon.isZeroSq(WB_CoordOp3D.getSqDistance3D(point, _bin[i].coord))) {
					return _bin[i].value;
				}
			}
			return Double.NaN;
		...
		private void findNearest(final QueryResultDouble<T> heap, final WB_Coord data) {
			...
			if (_isLeaf) {
				for (int i = 0; i < _binSize; i++) {
					final double dist = WB_CoordOp3D.getSqDistance3D(_bin[i].coord, data);
		 ...
		 * @return
		 */
		private WB_KDEntryDouble<T>[] range(final WB_AABB range) {
			if (_bin == null) {
				@SuppressWarnings("unchecked")
		...
		private WB_KDEntryDouble<T>[] range(final WB_Coord center, final double r2) {
			...
			int n = 0;
			for (int i = 0; i < _binSize; i++) {
				final double d2 = WB_CoordOp3D.getSqDistance3D(center, _bin[i].coord);
				if (d2 <= r2) {
					_bin[i].d2 = d2;
					tmp[n++] = _bin[i];
				}
		...
		private void addLevel() {
			...
			_negative = new WB_KDNodeDouble<T>();
			_positive = new WB_KDNodeDouble<T>();
			_negative._id = 2 * _id;
