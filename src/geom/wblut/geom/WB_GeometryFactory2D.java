...
public class WB_GeometryFactory2D {
	...
	public static WB_GeometryFactory2D instance() {
		return new WB_GeometryFactory2D();
	 ...
	 */
	public WB_CoordinateSystem3D WORLD() {
		return WB_CoordinateSystem3D.WORLD();
	}
	 ...
	 * @return coordinate
	 */
	public WB_CoordinateSystem3D createCSFromOX2D(final WB_Coord origin, final WB_Coord X) {
		...
		return createCSFromOXY2D(lOrigin, lX, lY);
	}
	...
	public WB_CoordinateSystem3D createCSFromOX2D(final WB_Coord origin, final WB_Coord X,
		...
		final WB_Vector lX = createNormalizedVector2D(X.xd(), X.yd());
		final WB_Vector lY = createVector2D(-lX.yd(), lX.xd());
		return createCSFromOXY2D(lOrigin, lX, lY, parent);
	}
	 ...
	 * @return coordinate system
	 */
	public WB_CoordinateSystem3D createCSFromOXY2D(final WB_Coord origin, final WB_Coord X, final WB_Coord Y) {
	 ...
	 * @return
	 */
	public WB_CoordinateSystem3D createTransformedCS(final WB_CoordinateSystem3D CS, final WB_Transform T,
			final WB_CoordinateSystem3D parent) {
	 ...
	 * @return
	 */
	public WB_Point createPoint2D(final Coordinate coord) {
	...
	public List<WB_Point> createIntersectionPoints2D(final WB_Line L, final WB_Circle C) {
		...
		final double c = WB_Point.getSqLength3D(C.getCenter()) + WB_Vector.getSqLength3D(L.getOrigin())
	...
	public WB_Point createIntersectionPoint2D(final WB_Line L1, final WB_Line L2) {
		...
		final WB_Vector r = createVector2D(WB_Vector.sub(L1.getOrigin(), L2.getOrigin()));
	...
	public WB_Point createMirrorPoint2D(final WB_Coord p, final WB_Line L) {
		...
		a = dx * dx - dy * dy;
		b = 2 * dx * dy;
		x2 = a * (p.xd() - L.getOrigin().xd()) + b * (p.yd() - L.getOrigin().yd()) + L.getOrigin().xd();
		y2 = b * (p.xd() - L.getOrigin().xd()) - a * (p.yd() - L.getOrigin().yd()) + L.getOrigin().yd();
		return createPoint2D(x2, y2);
	}
	...
	public WB_Point createInversionPoint2D(final WB_Coord p, final WB_Circle inversionCircle) {
		...
		final WB_Vector v = createNormalizedVectorFromTo2D(inversionCircle.getCenter(), p);
	...
	public WB_Point createOrthocenter2D(final WB_Triangle tri) {
		...
		return createPointFromBarycentricCoordinates2D((a2 + b2 - c2) * (a2 - b2 + c2),
	...
	public List<WB_Line> createAngleBisector2D(final WB_Line L1, final WB_Line L2) {
		...
		if (intersection == null) {
			final WB_Point L1onL2 = createClosestPointOnLine2D(L1.getOrigin(), L2);
			result.add(createLineWithDirection2D(L1onL2.mulAddMul(0.5, 0.5, L1.getOrigin()), L1.getDirection()));
			return result;
		} else {
			if (WB_Vector.dot(L1.getDirection(), L2.getDirection()) > 0) {
				...
				final WB_Vector dir = createVector2D(p1.mulAddMul(0.5, 0.5, p2).sub(intersection));
				result.add(createLineWithDirection2D(intersection, dir));
				result.add(createLineWithDirection2D(intersection.xd(), intersection.yd(), -dir.yd(), dir.xd()));
				return result;
			} else {
				final WB_Point p1 = intersection.addMul(100, L1.getDirection());
				final WB_Point p2 = intersection.addMul(-100, L2.getDirection());
	...
	public List<WB_Line> createLinesTangentToCircleThroughPoint(final WB_Circle C, final WB_Coord p) {
		...
		if (WB_Epsilon.isZero(dcp - C.getRadius())) {
			...
			result.add(createLineWithDirection2D(p,
					createVector2D(-(r2 * u.yd() + rad) / denom, (r2 - (num + u.yd() * rad) / denom) / u.xd())));
			result.add(createLineWithDirection2D(p,
					createVector2D(-(r2 * u.yd() - rad) / denom, (r2 - (num - u.yd() * rad) / denom) / u.xd())));
		} else {
			...
			final double rad = Math.sqrt(-r4 * ux2 + r2 * ux4 + r2 * ux2 * uy2);
			result.add(createLineWithDirection2D(p,
					createVector2D((r2 - (num + u.xd() * rad) / denom) / u.yd(), -(r2 * u.xd() + rad) / denom)));
			result.add(createLineWithDirection2D(p,
					createVector2D((r2 - (num - u.xd() * rad) / denom) / u.yd(), -(r2 * u.xd() - rad) / denom)));
		}
		return result;
	...
	public List<WB_Line> createLinesTangentTo2Circles(final WB_Circle C0, final WB_Circle C1) {
		...
		if (!WB_Epsilon.isZero(rdiff)) {
			...
			WB_Point org = createPoint2D(C0.getCenter().xd() + s * w.xd(), C0.getCenter().yd() + s * w.yd());
			...
			org = createPoint2D(C0.getCenter().xd() + s * w.xd(), C0.getCenter().yd() + s * w.yd());
			if (!inside) {
				result.add(createLineWithDirection2D(org, dir.get(0)));
			}
			if (!inside) {
			...
			final WB_Point mid = createPoint2D(C0.getCenter()).mulAddMul(0.5, 0.5, C1.getCenter());
			final double a = Math.sqrt(WB_Math.fastAbs(wlensqr - 4 * C0.getRadius() * C0.getRadius()));
			final double invwlen = 1.0 / Math.sqrt(wlensqr);
			result.add(createLineWithDirection2D(createPoint2D(mid.xd() + C0.getRadius() * w.yd() * invwlen,
		...
		if (WB_Epsilon.isZeroSq(wlensqr - rsum * rsum)) {
			final WB_Point org = createInterpolatedPoint2D(C0.getCenter(), C1.getCenter(),
					C0.getRadius() / (C0.getRadius() + C1.getRadius()));
			final WB_Vector dir = createNormalizedVector2D(C1.getCenter().xd() - C0.getCenter().xd(),
					C1.getCenter().yd() - C0.getCenter().yd());
			result.add(createLineWithDirection2D(org, createVector2D(-dir.yd(), dir.xd())));
		}
	...
	public List<WB_Line> createPerpendicularLinesTangentToCircle(final WB_Line L, final WB_Circle C) {
		...
		result.add(createLineWithDirection2D(
				createPoint2D(C.getCenter().xd() + C.getRadius() * L.getDirection().xd(),
	...
	public WB_Polygon createSimplePolygon(final WB_Polygon poly) {
		return new WB_PolygonTriangulatorJTS().makeSimplePolygon(poly);
	}
	...
	public WB_Polygon createSimplePolygon(final WB_Quad quad) {
		return new WB_Polygon(quad.p1, quad.p2, quad.p3, quad.p4);
	}

	public WB_Polygon createSimplePolygon(final WB_Pentagon pentagon) {
		return new WB_Polygon(pentagon.p1, pentagon.p2, pentagon.p3, pentagon.p4, pentagon.p5);
	}

	public WB_Polygon createSimplePolygon(final WB_Hexagon hexagon) {
		return new WB_Polygon(hexagon.p1, hexagon.p2, hexagon.p3, hexagon.p4, hexagon.p5, hexagon.p6);
	}

	public WB_Polygon createSimplePolygon(final WB_Octagon octagon) {
		return new WB_Polygon(octagon.p1, octagon.p2, octagon.p3, octagon.p4, octagon.p5, octagon.p6, octagon.p7,
				octagon.p8);
	}
	 ...
	 * @return
	 */
	public WB_Polygon createPolygonFromJTSPolygon2D(final Polygon JTSpoly) {
	...
	public WB_Polygon createPolygonConvexHull2D(final WB_Polygon poly) {
		...
		if (result.getGeometryType().equals("Polygon")) {
			createPolygonFromJTSPolygon2D((Polygon) result);
		}
		return null;
	}
	...
	public List<WB_Polygon> createBoundaryPolygons2D(final Collection<? extends WB_Polygon> poly) {
		...
		if (poly.size() < 1) {
			...
			while (pItr.hasNext()) {
				result = unionPolygons2D(result, createBoundaryPolygons2D(pItr.next()));
	 ...
	 * @return
	 */
	Polygon toJTSPolygon2D(final WB_Polygon poly) {
	...
	Geometry toJTSMultiPolygon2D(final List<WB_Polygon> polys) {
	 ...
	 * @return
	 */
	Coordinate toJTSCoordinate2D(final WB_Coord point, final int i) {
	...
	public List<WB_Circle> createCirclePPL(final WB_Coord p, final WB_Coord q, final WB_Line L) {
		...
		if (F == null) {
			...
			final List<WB_Point> intersections = createIntersectionPoints2D(STF, OPQ);
	...
	public List<WB_Circle> createCirclePLL(final WB_Coord p, final WB_Line L1, final WB_Line L2) {
		...
		if (A == null) {
			...
			for (final WB_Circle C : selcircles) {
				...
				final List<WB_Point> intersections = createIntersectionPoints2D(Ap, C);
	...
	public List<WB_Circle> createCirclePPC(final WB_Coord p, final WB_Coord q, final WB_Circle C) {
		...
		if (pType == WB_Classification.ON && qType == WB_Classification.ON) {
			...
			if (center != null) {
				result.add(createCircleWithRadius(center, center.getDistance2D(p)));
			}
			return result;
		...
		// Both points outside
			...
			final WB_Line ABbis = createBisector2D(p, q);
			if (WB_Point.isCollinear(C.getCenter(), ABbis.getOrigin(), ABbis.getPoint(100.0))) {
				final List<WB_Point> points = createIntersectionPoints2D(ABbis, C);
				for (final WB_Point pt : points) {
					result.add(createCirclePPP(pt, p, q));
				}
			} else {
				WB_Vector v = createVector2D(-AB.getDirection().yd(), AB.getDirection().xd());
				WB_Point E = WB_Point.addMul(C.getCenter(), 0.5 * C.getRadius(), v);
				if (E.isCollinear(p, q)) {
					v = createVector2D(AB.getDirection().yd(), -AB.getDirection().xd());
					E = WB_Point.addMul(C.getCenter(), 0.5 * C.getRadius(), v);
				}
				final WB_Circle circle = createCirclePPP(p, q, E);
				final List<WB_Point> intersections = createIntersectionPoints2D(circle, C);
				final WB_Line MN = createLineThroughPoints2D(intersections.get(0), intersections.get(1));
				final WB_Point point = createIntersectionPoint2D(AB, MN);
				if (point == null) {
					return result;
				}
				final List<WB_Line> tangents = createLinesTangentToCircleThroughPoint(C, point);
				for (final WB_Line L : tangents) {
					final WB_Point T = createClosestPointOnLine2D(C.getCenter(), L);
					final WB_Line ATbis;
			...
			if (WB_Point.isCollinear(iC2.getCenter(), ABbis.getOrigin(), ABbis.getPoint(100.0))) {
				final List<WB_Point> points = createIntersectionPoints2D(ABbis, iC2);
				for (final WB_Point pt : points) {
				...
				WB_Vector v = createVector2D(-AB.getDirection().yd(), AB.getDirection().xd());
				WB_Point E = WB_Point.addMul(iC2.getCenter(), 0.5 * iC2.getRadius(), v);
				if (E.isCollinear(ip, iq)) {
					v = createVector2D(AB.getDirection().yd(), -AB.getDirection().xd());
					E = WB_Point.addMul(iC2.getCenter(), 0.5 * iC2.getRadius(), v);
				}
				final WB_Circle circle = createCirclePPP(ip, iq, E);
				final List<WB_Point> intersections = createIntersectionPoints2D(circle, iC2);
				final WB_Line MN = createLineThroughPoints2D(intersections.get(0), intersections.get(1));
				final WB_Point point = createIntersectionPoint2D(AB, MN);
				if (point == null) {
					return result;
				}
				final List<WB_Line> tangents = createLinesTangentToCircleThroughPoint(iC2, point);
				for (final WB_Line L : tangents) {
					final WB_Point T = createClosestPointOnLine2D(iC2.getCenter(), L);
					final WB_Line ATbis;
					ATbis = createBisector2D(ip, T);
					final WB_Point center = createIntersectionPoint2D(ABbis, ATbis);
					if (center != null) {
						iresult.add(createCircleWithRadius(center, center.getDistance2D(ip)));
					}
				}
			}
			for (final WB_Circle circle : iresult) {
				...
				result.add(createCircleWithRadius(
						...
						iC.getCenter().yd() + s * (circle.getCenter().yd() - iC.getCenter().yd()),
	...
	public List<WB_Circle> createCirclePCC(final WB_Coord p, final WB_Circle C1, final WB_Circle C2) {
		...
		// completely outside
			...
			if (tangents.size() > 1) {
				...
				if (point != null) {
					final WB_Point G = createClosestPointOnLine2D(C1.getCenter(), T1);
					final WB_Point H = createClosestPointOnLine2D(C2.getCenter(), T1);
					final WB_Circle circle = createCirclePPP(G, H, p);
					final WB_Line Pp = createLineThroughPoints2D(p, point);
					final List<WB_Point> intersections = createIntersectionPoints2D(Pp, circle);
					WB_Point Ep = null;
					if (!WB_Epsilon.isZero(intersections.get(0).getDistance2D(p))) {
						Ep = intersections.get(0);
					} else if (!WB_Epsilon.isZero(intersections.get(1).getDistance2D(p))) {
					...
					if (intersections.size() == 2) {
						...
						final WB_Line L = createLineThroughPoints2D(C1.getCenter(), C2.getCenter());
		...
		// C1 and C2 crossing and p in only one of the two circles
			...
			if (WB_GeometryOp2D.classifyPointToCircle2D(p, C1) == WB_Classification.INSIDE) {
				...
				if (r1 <= r2) {
					...
					final WB_Point center = createPoint2D(p).addMulSelf(0.45 * r1, v);
					invC = createCircleWithRadius(center, 0.45 * r1);
				} else {
					final WB_Coord v = createLineThroughPoints2D(C2.getCenter(), p).getDirection();
					final WB_Point center = createPoint2D(p).addMulSelf(0.45 * r2, v);
					invC = createCircleWithRadius(center, 0.45 * r2);
				}
				final WB_Circle invC1 = createInversionCircle2D(C1, invC);
				final WB_Circle invC2 = createInversionCircle2D(C2, invC);
				if (invC1 != null) {
					final List<WB_Circle> invResult = createCirclePCC(p, invC1, invC2);
					for (final WB_Circle inv : invResult) {
						result.add(createInversionCircle2D(inv, invC));
					}
				}
		...
		if (WB_GeometryOp2D.classifyPointToCircle2D(p, C) == WB_Classification.ON) {
			...
			if (WB_GeometryOp2D.classifyPointToLine2D(p, Lperp) != WB_Classification.ON) {
				...
				if (WB_Epsilon.isZero(A2.getDistance2D(B))) {
					...
					if (!WB_Epsilon.isZeroSq(points.get(0).getSqDistance(A1))) {
						Q1 = points.get(0);
					} else if (!WB_Epsilon.isZeroSq(points.get(1).getSqDistance(A1))) {
						Q1 = points.get(1);
					}
					if (Q1 != null) {
						Lperp = createPerpendicularLineThroughPoint2D(L, p);
						final WB_Line Q1P = createBisector2D(Q1, p);
						final WB_Point intersection = createIntersectionPoint2D(Lperp, Q1P);
						result.add(createCircleWithRadius(intersection, intersection.getDistance2D(Q1)));
					}
					...
					if (!WB_Epsilon.isZeroSq(points.get(0).getSqDistance(p))) {
						Q1 = points.get(0);
					} else if (!WB_Epsilon.isZeroSq(points.get(1).getSqDistance(p))) {
						Q1 = points.get(1);
					}
					if (Q1 != null) {
			...
			if (tangentcircle) {
				...
				if (WB_GeometryOp2D.classifyPointToLine2D(p, L) == WB_GeometryOp2D.classifyPointToLine2D(C.getCenter(),
					...
					p1 = createInversionPoint2D(WB_Point.addMul(C.getCenter(), C.getRadius(), X2D()), inversion);
					p2 = createInversionPoint2D(WB_Point.addMul(C.getCenter(), -C.getRadius(), X2D()), inversion);
					final WB_Point p3 = createInversionPoint2D(WB_Point.addMul(C.getCenter(), C.getRadius(), Y2D()),
							inversion);
	...
	public List<WB_Circle> createCircleLLC(final WB_Line L1, final WB_Line L2, final WB_Circle C) {
		...
		if (A == null) {
			...
			final List<WB_Point> points = createIntersectionPoints2D(bis.get(0), C);
			points.addAll(createIntersectionPoints2D(bis.get(1), C));
			for (final WB_Point point : points) {
				result.addAll(createCirclePLL(point, L1, L2));
			}
		} else {
			final WB_Vector v1 = createVector2D(-L1.getDirection().yd(), L1.getDirection().xd());
			final WB_Vector v2 = createVector2D(-L2.getDirection().yd(), L2.getDirection().xd());
			WB_Line L1s = createLineWithDirection2D(new WB_Point(L1.getOrigin()).addMulSelf(-C.getRadius(), v1),
					L1.getDirection());
			WB_Line L2s = createLineWithDirection2D(new WB_Point(L2.getOrigin()).addMulSelf(-C.getRadius(), v2),
					L2.getDirection());
			...
			L1s = createLineWithDirection2D(new WB_Point(L1.getOrigin()).addMulSelf(C.getRadius(), v1),
					L1.getDirection());
			L2s = createLineWithDirection2D(new WB_Point(L2.getOrigin()).addMulSelf(C.getRadius(), v2),
					L2.getDirection());
	...
	public List<WB_Circle> createCircleLCC(final WB_Line L, final WB_Circle C1, final WB_Circle C2) {
		...
		if (C1.getRadius() == C2.getRadius()) {
			final WB_Vector v = createVector2D(-L.getDirection().yd(), L.getDirection().xd());
			final WB_Line L1s = createLineWithDirection2D(WB_Vector.addMul(L.getOrigin(), -C1.getRadius(), v),
					L.getDirection());
			final WB_Line L2s = createLineWithDirection2D(WB_Vector.addMul(L.getOrigin(), C1.getRadius(), v),
					L.getDirection());
			tmp = createCirclePPL(C1.getCenter(), C2.getCenter(), L1s);
			tmp.addAll(createCirclePPL(C1.getCenter(), C2.getCenter(), L2s));
			createCircleWithRadius(C1.getCenter(), C1.getRadius() + C2.getRadius());
			tmp.addAll(createCirclePLC(C1.getCenter(), L1s,
					createCircleWithRadius(C2.getCenter(), C1.getRadius() + C2.getRadius())));
			tmp.addAll(createCirclePLC(C1.getCenter(), L2s,
					createCircleWithRadius(C2.getCenter(), C1.getRadius() + C2.getRadius())));
			tmp.addAll(createCirclePLC(C2.getCenter(), L1s,
					createCircleWithRadius(C1.getCenter(), C1.getRadius() + C2.getRadius())));
			tmp.addAll(createCirclePLC(C2.getCenter(), L2s,
					createCircleWithRadius(C1.getCenter(), C1.getRadius() + C2.getRadius())));
		} else {
			WB_Circle Cm;
			WB_Circle CM;
			...
			WB_Circle C = createCircleWithRadius(CM.getCenter(), CM.getRadius() - Cm.getRadius());
			final WB_Vector v = createVector2D(-L.getDirection().yd(), L.getDirection().xd());
			final WB_Line L1s = createLineWithDirection2D(WB_Vector.addMul(L.getOrigin(), -Cm.getRadius(), v),
					L.getDirection());
			final WB_Line L2s = createLineWithDirection2D(WB_Vector.addMul(L.getOrigin(), Cm.getRadius(), v),
					L.getDirection());
			tmp = createCirclePLC(Cm.getCenter(), L1s, C);
			tmp.addAll(createCirclePLC(Cm.getCenter(), L2s, C));
			C = createCircleWithRadius(CM.getCenter(), CM.getRadius() + Cm.getRadius());
	...
	public List<WB_Circle> createCircleCCC(WB_Circle C1, WB_Circle C2, WB_Circle C3) {
		...
		if (C1.getRadius() == C2.getRadius() && C2.getRadius() == C3.getRadius()) {
			final double R = C1.getRadius();
			circles.add(createCirclePPP(C1.getCenter(), C2.getCenter(), C3.getCenter()));
			circles.addAll(
					createCirclePPC(C1.getCenter(), C2.getCenter(), createCircleWithRadius(C3.getCenter(), 2 * R)));
			circles.addAll(
					createCirclePPC(C1.getCenter(), C3.getCenter(), createCircleWithRadius(C2.getCenter(), 2 * R)));
			circles.addAll(
					createCirclePPC(C3.getCenter(), C2.getCenter(), createCircleWithRadius(C1.getCenter(), 2 * R)));
			for (final WB_Circle circle : circles) {
				result.add(createCircleWithRadius(circle.getCenter(), circle.getRadius() + R));
				result.add(createCircleWithRadius(circle.getCenter(), circle.getRadius() - R));
			}
			...
			circles.addAll(createCirclePPC(C1.getCenter(), C2.getCenter(),
					createCircleWithRadius(C3.getCenter(), C3.getRadius() - R)));
			circles.addAll(createCirclePCC(C1.getCenter(), createCircleWithRadius(C2.getCenter(), 2 * R),
					createCircleWithRadius(C3.getCenter(), C3.getRadius() + R)));
			circles.addAll(createCirclePCC(C1.getCenter(), createCircleWithRadius(C2.getCenter(), 2 * R),
					createCircleWithRadius(C3.getCenter(), C3.getRadius() - R)));
			for (final WB_Circle circle : circles) {
			...
			circles.addAll(createCirclePCC(C1.getCenter(), createCircleWithRadius(C2.getCenter(), C2.getRadius() + R),
					createCircleWithRadius(C3.getCenter(), C3.getRadius() + R)));
			circles.addAll(createCirclePCC(C1.getCenter(), createCircleWithRadius(C2.getCenter(), C2.getRadius() + R),
					createCircleWithRadius(C3.getCenter(), C3.getRadius() - R)));
			circles.addAll(createCirclePCC(C1.getCenter(), createCircleWithRadius(C2.getCenter(), C2.getRadius() - R),
			...
			R = C2.getRadius();
			circles.addAll(createCirclePCC(C2.getCenter(), createCircleWithRadius(C1.getCenter(), C1.getRadius() + R),
					createCircleWithRadius(C3.getCenter(), C3.getRadius() + R)));
			circles.addAll(createCirclePCC(C3.getCenter(), createCircleWithRadius(C1.getCenter(), C1.getRadius() + R),
	...
	public List<WB_Circle> createCircleTangentToLineThroughPoint(final WB_Line L, final WB_Coord p, final double r) {
		...
		if (WB_Epsilon.isZero(cPrime)) {
			result.add(createCircleWithRadius(createPoint2D(p.xd() + r * L.a(), p.yd() + r * L.b()), r));
			result.add(createCircleWithRadius(createPoint2D(p.xd() - r * L.a(), p.yd() - r * L.b()), r));
			return result;
		}
		double a, b;
		...
		if (WB_Epsilon.isZero(tmp2)) {
			result.add(createCircleWithRadius(createPoint2D(p.xd() - tmp1 * a, p.yd() - tmp1 * b), r));
			return result;
		} else if (tmp2 < 0) {
			return result;
		} else {
			tmp2 = Math.sqrt(tmp2);
			final WB_Point tmpp = createPoint2D(p.xd() - a * tmp1, p.yd() - b * tmp1);
			result.add(createCircleWithRadius(createPoint2D(tmpp.xd() + tmp2 * b, tmpp.yd() - tmp2 * a), r));
			result.add(createCircleWithRadius(createPoint2D(tmpp.xd() - tmp2 * b, tmpp.yd() + tmp2 * a), r));
			return result;
		}
	}
	...
	public List<WB_Circle> createCircleTangentTo2Lines(final WB_Line L0, final WB_Line L1, final double r) {
		...
		double cx = -(L1.b() * (L0.c() + discrm0) - L0.b() * (L1.c() + discrm1)) * invDenom;
		double cy = +(L1.a() * (L0.c() + discrm0) - L0.a() * (L1.c() + discrm1)) * invDenom;
		result.add(createCircleWithRadius(createPoint2D(cx, cy), r));
		cx = -(L1.b() * (L0.c() + discrm0) - L0.b() * (L1.c() - discrm1)) * invDenom;
		cy = +(L1.a() * (L0.c() + discrm0) - L0.a() * (L1.c() - discrm1)) * invDenom;
		result.add(createCircleWithRadius(createPoint2D(cx, cy), r));
		cx = -(L1.b() * (L0.c() - discrm0) - L0.b() * (L1.c() + discrm1)) * invDenom;
		cy = +(L1.a() * (L0.c() - discrm0) - L0.a() * (L1.c() + discrm1)) * invDenom;
		result.add(createCircleWithRadius(createPoint2D(cx, cy), r));
	...
	public List<WB_Circle> createCircleTangentToLineAndCircle(final WB_Line L, final WB_Circle C, final double r) {
		...
		final WB_Line L1 = createLineWithDirection2D(
				WB_Vector.addMul(L.getOrigin(), r, createVector2D(L.getDirection().yd(), -L.getDirection().xd())),
				L.getDirection());
		final WB_Line L2 = createLineWithDirection2D(
				WB_Vector.addMul(L.getOrigin(), r, createVector2D(-L.getDirection().yd(), +L.getDirection().xd())),
				L.getDirection());
		final WB_Circle C1 = createCircleWithRadius(C.getCenter(), C.getRadius() + r);
		final WB_Circle C2 = createCircleWithRadius(C.getCenter(), WB_Math.fastAbs(C.getRadius() - r));
	...
	public List<WB_Circle> createCircleTangentTo2CirclesNonCollinear(final WB_Circle C0, final WB_Circle C1,
		...
		final WB_Circle C0r = createCircleWithRadius(C0.getCenter(), C0.getRadius() + r);
		final WB_Circle C1r = createCircleWithRadius(C1.getCenter(), C1.getRadius() + r);
		final List<WB_Point> intersections = createIntersectionPoints2D(C0r, C1r);
		if (intersections.size() < 2) {
			return result;
		}
		for (int i = 0; i < intersections.size(); i++) {
	...
	public List<WB_Circle> createCircleTangentTo2CirclesCollinear(final WB_Circle C0, final WB_Circle C1) {
		...
		double f = (r - C0.getRadius()) / (2 * r - C0.getRadius() - C1.getRadius());
		if (!WB_Epsilon.isZero(r)) {
			result.add(createCircleWithRadius(createInterpolatedPoint2D(C0.getCenter(), C1.getCenter(), f),
					WB_Math.fastAbs(r)));
		}
		...
		f = (r - C1.getRadius()) / d;
		if (!WB_Epsilon.isZero(r)) {
			result.add(createCircleWithRadius(createInterpolatedPoint2D(C1.getCenter(), C0.getCenter(), f),
					WB_Math.fastAbs(r)));
		}
		r = 0.5 * (d - C1.getRadius() - C0.getRadius());
		f = (r + C0.getRadius()) / d;
	...
	public WB_Point createClosestPointOnPolygon2D(final WB_Coord p, final WB_Polygon poly) {
		...
		for (int i = 0; i < n; i += 3) {
			tmp = createClosestPointOnTriangle2D(p, poly.getPoint(triangles[i]), poly.getPoint(triangles[i + 1]),
