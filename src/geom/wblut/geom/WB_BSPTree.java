...
public class WB_BSPTree {
	 ...
	 * @return the int
	 */
	private int pointLocation(final WB_BSPNode node, final WB_Coord p) {
		final WB_Classification type = WB_GeometryOp3D.
	...
	private void partitionPolygon(final WB_BSPNode node,
		...
		final WB_Classification type = 
				WB_GeometryOp3D.classifyPolygonToPlane3D(P,node.partition);
...
WB_Polygon[] polygons =new WB_Polygon[2];
	...
	public static void main(String [] args)
	{
		 ...
		 HE_Mesh  mesh=new HE_Mesh(creator); 
		  ...
		  WB_BSPTree tree=new WB_BSPTree();
		  tree.build(mesh);
