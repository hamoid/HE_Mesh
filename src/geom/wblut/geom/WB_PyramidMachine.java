...
public class WB_PyramidMachine {
	...
	public WB_Mesh createPyramidWithAngle(final double... angles) {
		...
		try {
			...
			output = new WB_Mesh(lpoints, faces);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public WB_Mesh createPyramidWithHeightAndAngle(final double height, final double... angles) {
		WB_Mesh output = new WB_Mesh();
		try {
	 ...
	 * @return
	 */
	public WB_Mesh createPyramidWithHeight(final double height) {
		WB_Mesh output = new WB_Mesh();
		try {
			WB_CoordCollection mappedPoints = points.map(map);
			final Corner[] corners = new Corner[points.size()];
	...
	public WB_Mesh createOffset(final double height, final double... offset) {
		WB_Mesh output = new WB_Mesh();
		if (offset.length == 0) {
			return output;
		}
