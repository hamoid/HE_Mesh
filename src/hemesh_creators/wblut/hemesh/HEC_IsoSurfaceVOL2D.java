...
public class HEC_IsoSurfaceVOL2D extends HEC_Creator {
	...
	private void getPolygons(final int i, final int j, final int cubeindex, final WB_Point offset) {
		...
		for (int t = 0; t < numtris; t++) {
			...
			final HE_Vertex v3 = getIsoVertex(indices[currentindex++], i, j, offset);
	...
	public static void main(final String[] args) {
		...
		for (int i = 0; i < num; i++) {
			WB_Point p1 = new WB_Point(Math.random() * resx, Math.random() * resy);
			WB_Point p2 = new WB_Point(Math.random() * resx, Math.random() * resy);
		...
		HEC_IsoSurfaceVOL2D iso = new HEC_IsoSurfaceVOL2D().setResolution(resx, resy).setValues(values).setSize(dx, dy)
